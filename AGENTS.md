...
- CRITICAL: **무엇을 지적할지도 LLM이 고르지 않는다.** 신호 탐지는 결정론적 SQL(`src/lib/signals/`)이 하고, 우선순위는 원화 영향도 점수가 정한다. LLM은 이미 선별된 신호를 받아 문장으로 옮길 뿐이다. 임계값은 `src/lib/signals/thresholds.ts` 한 파일에만 둘 것. 이유: 선별까지 LLM에 맡기면 결과가 재현되지 않아 테스트할 수 없고, 근거 없는 지적을 추측으로 채우게 된다
...
`scripts/execute.py` 는 phase 안의 step 을 순차 실행하는 하네스다. 각 step 은 `codex exec` 세션 하나로 처리된다.

python3 scripts/execute.py <phase-dir>          # 예: 0-mvp
