...
색·타이포·간격·컴포넌트는 **`docs/DESIGN.md` 가 단일 출처다.** 토큰은 `src/app/globals.css` 한 파일에 CSS 변수로 두고, 라이트를 `:root` 에, 다크를 `[data-theme="dark"]` 에 정의한다. **컴포넌트에 hex를 직접 쓰지 않는다.** 카테고리 enum과 색 매핑은 `src/lib/categories.ts` 한 곳에서 나온다.
